...
var StockService = (function () {
    ...
    StockService.prototype.getStocks = function (symbols) {
        var params = new http_1.URLSearchParams();
        params.set('symbols', symbols.toString());
        console.log("stock service :  getStocks" + params.toString());
        return this.http.get('http://localhost:3000/api/stocks', { search: params }).map(function (res) {
            return res.json();
        });
    };
    StockService.prototype.addTask = function (task) {
        var headers = new http_1.Headers();
        headers.append('Content-Type', 'application/json');
        return this.http.post('http://localhost:3000/api/task', JSON.stringify(task), { headers: headers })
            .map(function (res) { return res.json(); });
    };
    StockService.prototype.deleteTask = function (id) {
